...
from mysql.FaceData import FaceData
import numpy as np
import torch
...
class Facenet(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": "model_data/backbone_weights_of_mobilenetv1.pth",
        # --------------------------------------------------------------------------#
        #   输入图片的大小。
        # --------------------------------------------------------------------------#
    ...
    def detect_image(self, image_1, image_2):
        ...
        with torch.no_grad():
            ...
            str = ''.join('%s' % id for id in output1)
            # print(str)
            # 负数消失
            ...
            #   计算二者之间的距离
            # ---------------------------------------------------#
            l1 = np.linalg.norm(new_numbers - output2, axis=1)
    ...
    def interFaceData(self, picurl, score):

        faceMySql = FaceData()
        faceMySql.inFaceURL(score=score, picurl=picurl)
        image_1 = Image.open(picurl)
        output1 =self.matrix_128(image_1)
        str = ''.join('%s' % id for id in output1)
        faceMySql.inFaceData(score, str)


    def outFaceData(self):
        faceMySql = FaceData()
        data=faceMySql.outFaceData()
        outdata=[]
        outlabel=[]
        for i in data:
            numbers = re.findall(r"\-?\d+\.\d*", i[1])
            new_numbers = []
            for n in numbers:
                new_numbers.append(float(n));
            aaaa = []
    ...
    def getImageLL(self,oldimg,newimg):
        return np.linalg.norm(oldimg - newimg, axis=1)